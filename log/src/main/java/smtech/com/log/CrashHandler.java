...
//		}
	...
	private boolean handleException(Throwable ex) {
		...
		final String msg = StackTraceToString.getExceptionTrace(ex);
		Log.e("CrashHandler",msg);
		// 使用Toast来显示异常信息
//		new Thread() {
//			@Override
//			public void run() {
...
//		}.start();
	...
	private void collectCrashDeviceInfo(Context ctx) {
		try {
			...
			logger.error(String.format("Error while collect package info,[%s]",
					StackTraceToString.getExceptionTrace(e)));
		}
		...
		for (Field field : fields) {
			try {
				...
				logger.error(String.format(
						"Error while collect crash info,[%s]",
						StackTraceToString.getExceptionTrace(e)));
			}
		}
	}
